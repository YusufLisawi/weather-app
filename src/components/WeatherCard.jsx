...
export default function WeatherCard({
  ...
  function cordsSaved(lat, lon) {
    return (
      pinnedCords.filter((c) => c.lat === lat && c.lon === lon).length > 0 ||
      nopin
    );
  }
    ...
    <>
      <Tilty max={6}>
        <motion.div
          ...
          style={{
            backgroundImage: `url("${imgUrl[details.toLowerCase()]}")`,
          }}
        >
          ...
          <div className="relative">
            ...
            >
              <div>
                <h1 className={`text-4xl font-semibold text-shadow`}>{name}</h1>
                <p className={`font-bold text-shadow opacity-60`}>
            ...
            <AnimatePresence>
              {expand && (
                ...
                >
                  <Forcast weather={weather.hourly} key="hourly" />
                  <Forcast weather={weather.daily} key="daily" />
                  <SunTime
                    sunrise={sunrise}
                    sunset={sunset}
                    timezone={timezone}
            ...
            <div className="font-medium flex justify-between items-center opacity-70">
              ...
              <p className="text-sm font-bold">H : {weather.humidity}%</p>
            </div>
          </div>
        </motion.div>
      </Tilty>
      <motion.div layout className="relative">
        {!cordsSaved(lat, lon) && (
          <p
            ...
            onClick={() => {
              dispatch(setPinnedCords({ name: name, lat: lat, lon: lon }));
              // navigate("pinned");
            }}
          >
            <TbPin size={20} />
        ...
        {remove && (
          <p
            className="absolute -top-9 -right-3 p-2 rounded-full bg-black hover:text-red-400 duration-300 cursor-pointer"
            onClick={() => {
              dispatch(deletePinnedCords(name));
              navigate("/");
            }}
          >
            <TbPinnedOff size={20} />
